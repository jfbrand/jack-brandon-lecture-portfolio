...
In this project, I primarily learned the effect of one dimension in the geometry of a design and how that effects other dimensions. For example, the values obtained for the feature 1 using yield strength and max deflection were relatively similar, as was the yield strength calculation for feature 2. However, due to the short length of the overhang and the small impact the moments had on that part of the mount, the minimum thickness was much smaller. Furthermore, I learned how a simple part such as this could become much more complicated with very minor details. When originally, setting up the equations, It was a challenge to pinpoint exactly where the moment arm was for the motor. I eventually decided to assume that the total shaft length would be a safe choice but I am still not sure if that holds true given the mounting location of the motor, no matter how slightly off it is. Lastly, it was very important to choose wisely which dimension to solve for in this project. Since the dimensions of the motor were known, the width of the feature 1 plate had account for the motor diameter. Due to this, I solved for the thickness instead. Had I done it the other way around, the mount likely would not have fit the motor properly without making adjustments to the design.
